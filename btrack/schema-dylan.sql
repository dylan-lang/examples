...
create unique index XPtbl_bug_report on tbl_bug_report ( record_id );


drop table tbl_bug_dependencies;
create table tbl_bug_dependencies (
  bug_report_id      INTEGER NOT NULL,
  depends_on         INTEGER NOT NULL       -- a bug report record_id
);
...
create table tbl_comment (
  ...
  date_entered       DATETIME NOT NULL,
  date_modified      DATETIME NOT NULL,
  bug_report_id      INTEGER NOT NULL,      -- references tbl_bug_report.record_id
  account_id         INTEGER NOT NULL,      -- references tbl_account.record_id
  comment            VARCHAR(4000)
);
create unique index XPtbl_comment on tbl_comment ( record_id );
...
create table tbl_version (
  ...
  product_id         INTEGER NOT NULL,      -- a product record_id
  release_date       DATETIME,
  comment            VARCHAR(4000)
);
create unique index XPtbl_version on tbl_version ( record_id );
...
create unique index XPtbl_operating_system on tbl_operating_system ( record_id );

-- Add or remove whatever you want from here. Just make sure the IDs are unique.
-- This doesn't try to be a complete list.
--insert into tbl_operating_system (record_id, name) values (1, 'Windows 95/98/Me');
--insert into tbl_operating_system (record_id, name) values (2, 'Windows NT4');
--insert into tbl_operating_system (record_id, name) values (3, 'Windows 2000');
--insert into tbl_operating_system (record_id, name) values (4, 'Windows XP');
--insert into tbl_operating_system (record_id, name) values (5, 'Linux');
--insert into tbl_operating_system (record_id, name) values (6, 'MacOS 8');
--insert into tbl_operating_system (record_id, name) values (7, 'MacOS 9');
--insert into tbl_operating_system (record_id, name) values (8, 'MacOS X');
--insert into tbl_operating_system (record_id, name) values (9, 'PS/2');
--insert into tbl_operating_system (record_id, name) values (10, 'Solaris');
...
create unique index XPtbl_browser on tbl_browser ( record_id );

-- Add or remove whatever you want from here.  Just make sure the IDs are unique.
-- This doesn't try to be a complete list.
--insert into tbl_browser (record_id, name) values (1, 'Internet Explorer 3');
--insert into tbl_browser (record_id, name) values (2, 'Internet Explorer 4');
--insert into tbl_browser (record_id, name) values (3, 'Internet Explorer 5');
--insert into tbl_browser (record_id, name) values (4, 'Internet Explorer 6');
--insert into tbl_browser (record_id, name) values (5, 'Netscape 3');
--insert into tbl_browser (record_id, name) values (6, 'Netscape 4');
--insert into tbl_browser (record_id, name) values (7, 'Netscape 6');
--insert into tbl_browser (record_id, name) values (8, 'Opera');
--insert into tbl_browser (record_id, name) values (9, 'ICab');
...
create table tbl_product (
  ...
  owner              INTEGER NOT NULL       -- The record_id of the account with primary responsibility.
);
create unique index XPtbl_product on tbl_product ( record_id );
...
create unique index XPtbl_module on tbl_module ( record_id );
...
create unique index xpktbl_account_id on tbl_account ( record_id );
...
create table tbl_log (
  record_id       INTEGER NOT NULL,
  date_entered    DATETIME NOT NULL,
  modified_by     INTEGER NOT NULL,        -- references tbl_account.record_id.  use string instead?
  description     VARCHAR(400) NOT NULL
);
create unique index XPtbl_log on tbl_log ( record_id );
...
create table tbl_config (
  ...
  next_record_id INTEGER NOT NULL,
...
insert into tbl_config ( next_record_id, next_bug_number ) values ( 100, 1 );
