...
define method generate-next-move(me :: <dumbot>, board :: <state>)
 => (c :: <command>)
  block(return)
    ...
    let packages-here = packages-at(board, me.robot.x, me.robot.y);
    if (!empty?(packages-here))
      let take-these = make(<vector>);
      let left = me.robot.capacity;
      for (x in sort(packages-here))
	if (x.weight <= me.robot.capacity
	      & path-between(me.robot.x, me.robot.y, x.x, y,y, board))
	  left := left - x.weight;
